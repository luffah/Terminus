new Builtin('cd', [ARGT.dir], function (args, env, sys) {
  ...
  if (args.length > 1) {
    ...
    if (cwd.room && cwd.room.checkAccess(env)) {
      env.previous = cwd
      return { stdout: _('cmd_cd_parent', Room.enter(cwd.room, env)), fireables: [[cwd.room, 0]] }
    ...
    const dest = env.traversee(args[0])
    const room = dest.room
    if (room) {
      if (room.checkAccess(env)) {
        const hret = room.tryhook('cd', args)
