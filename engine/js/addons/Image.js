  ...
  this.on('Render', function (ret) {
    if (ret.render) {
      ...
      if (ret.render instanceof File) {
        ...
        var pics = []
        for (const f of ret.render.leafs) {
...
VTermImages.prototype = {
  ...
  showImgs (idx = -1) {
    const v = this
    const imgs = v.imgs[idx]
    if (imgs && imgs.length > 0) {
      const c = addEl(v.monitor, 'div', 'img-container')
      while (imgs.length) {
  ...
  rmCurrentImg (timeout) {
    const v = this
    setTimeout(function () {
      for (const el of v.current_msg.getElementsByClassName('img-container')) {
...
class Pic {
  ...
  get src () {
    const res = this.bank[this.o.img]
    if (res) {
      res.used = true
      return res.src
    }
  ...
  renderAsChild (cont, cnt, onload) {
    ...
    if (t.exists()) {
      ...
      for (const childpic of t.children) {
  ...
  render (c, onload) {
    const t = this
    if (t.exists()) {
      const cont = addEl(c, 'div', picturable({ class: 'layers' }))
      const picl = new PicLayers(t, cont, onload)
      picl.update()
...
class PicLayers {
  ...
  update () {
    const t = this
    const cont = t.cont; const onload = t.onload
    cont.innerHTML = ''
    cont.className = 'layers ' + t.othercls + (t.reverseX ? ' reverseX' : '') + (t.reverseY ? ' reverseY' : '')
    const over = addEl(cont, 'div', { class: 'foreground' })
    const behind = addEl(cont, 'div', { class: 'background' })
    ...
    if (t.pic.o.img) {
      ...
      const img = addEl(cont, 'div', { class: 'main ' + (t.pic.cls || '') + ' ' + (t.pic.tmpcls || '') })
      adaptHeight = (e) => {
    ...
    delete t.pic.tmpcls
    let cnt = 0
    for (const childpic of t.pic.children) {
