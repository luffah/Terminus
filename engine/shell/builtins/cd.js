Builtin.def('cd', [ARGT.dir], function (args, ctx, vt) {
  ...
  if (args.length > 1) {
    ...
    return cmdDone(vt, [[cwd.previous, 0]], {}, 'cd', args)
  } else if (args[0] === '..') {
    cwd.fire(vt, 'cd', args, 0)
    ...
    if (home && home.checkAccess(ctx)) {
      ctx.previous = cwd
      Room.enter(home, vt)
      return cmdDone(vt, [[home, 0]], _stdout(_('cmd_cd_home')), 'cd', args)
    ...
    if (room) {
      if (room.checkAccess(ctx)) {
        ...
        if (!dest.item_name) {
          ctx.previous = cwd
          return cmdDone(vt, [[room, 0]], _stdout(_('cmd_cd', Room.enter(room, vt))), 'cd', args)
        }
      } else {
        cwd.fire(vt, 'cd', args, 0, { 'unreachable_room': room })
