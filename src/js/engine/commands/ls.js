...
function printLS (room, path, opt) {
  if (opt.l) {
  ...
  let prerender = (t, f, i) => {
    if (f.picture && f.picture.shown_in_ls && (!(t == 'room') || f.pic_shown_as_item)) {
      f.picture.setOneShotRenderClass(render_classes[t])
      pics[t + '-' + i] = f.picture
    }
  }
...
_defCommand('ls', [ARGT.dir], function (args, ctx, vt) {
  ...
  if (files.length) {
    let room = ctx.traversee(files[0]).room
    if (room) {
      ...
      if (room.isEmpty()) {
        ...
        prtls = printLS(room, files[0], opt)
      }
