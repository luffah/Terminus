...
function _expandArgs (args, ctx) {
  ...
  args.forEach((arg) => {
    if (regexpStr.test(arg)) {
      ...
      let [room, lastcomponent, path] = ctx.h.r.pathToRoom(arg)
...
function _completeArgs (args, argidx, tocomplete, ctx, compl) { // return completion matches
  let roomCurrent = tocomplete.substring(0, 1) == '~' ? $home : ctx.h.r
...
function _parse_command (vt, line) {
  let arrs = line.split(' ').filter((s) => s.length)
  let ctx = vt.ctx
  let cmdname = arrs[0]
  let r = ctx.h.r
  let ret = ''
  ...
  let result = cmd.exec(args, ctx, vt)
  if (result) {
    ret = result
